...
static gboolean gvm_local_user (void);
static gboolean gvm_active_user (void);
...
gvm_prompt (GvmPromptCtx *ctx, int argc, char **argv)
{
	...
	if ((pixbuf = gtk_icon_theme_load_icon (theme, gvm_prompts[prompt].icon, 48, 0, NULL))) {
...
autorun_cb (GvmPromptCtx *ctx, int action)
{
	...
	if (action == GVM_RESPONSE_RUN) {
		argv[0] = ctx->path;
		argv[1] = NULL;
		
		g_spawn_async (ctx->mount_point, argv, NULL,
...
static void
autoopen_cb (GvmPromptCtx *ctx, int action)
{
	gboolean autobrowse = TRUE;
	GError *error = NULL;
	char *argv[3];
	
	if (action == GVM_RESPONSE_OPEN) {
		argv[0] = "gnome-open";
		argv[1] = ctx->path;
		argv[2] = NULL;
...
gvm_autorun (const char *udi, const char *device, const char *mount_point)
{
	...
	if (config.autoopen && config.autoopen_path) {
		...
		for (i = 0; autopath[i]; i++) {
			char *outbuf = NULL, *outptr = NULL, buf[256];
			gboolean eoln = FALSE;
			size_t n, m;
			FILE *fp;
			...
			while (!eoln && fgets (buf, sizeof (buf), fp)) {
				...
				if (outbuf || !eoln) {
					if (outbuf) {
						m = outptr - outbuf;
						outbuf = g_realloc (outbuf, m + n + 1);
						outptr = g_stpcpy (outbuf + m, buf);
					} else {
						outbuf = g_malloc (n + 1);
						outptr = g_stpcpy (outbuf, buf);
					}
			...
			path = g_build_filename (mount_point, path, NULL);
			g_free (outbuf);
			
			if (strncmp (canon_path (path), mount_point, strlen (mount_point) != 0)) {
				/* not allowed */
				g_free (path);
				break;
			}
			...
			argv[0] = autopath[i];
			ctx = gvm_prompt_ctx_new (GVM_PROMPT_AUTOOPEN, autoopen_cb, udi, device, mount_point, path);
			gvm_prompt (ctx, 1, argv);
			handled = TRUE;
			g_free (path);
			break;
...
static gboolean
camera_device_added (const char *udi, const char *capability __attribute__((__unused__)))
{
	GvmPromptCtx *ctx;
...
static gboolean
pda_device_added (const char *udi, const char *capability __attribute__((__unused__)))
{
	DBusError error;
	char *platform;
...
static gboolean
media_player_device_added (const char *udi, const char *capability __attribute__((__unused__)))
{
	char *access_method;
	DBusError error;
...
gvm_hal_init (void)
{
	...
	if (!libhal_ctx_init (ctx, &error)) {
		warn ("libhal_ctx_init failed: %s", error.message);
		dbus_error_free (&error);
		libhal_ctx_free (ctx);
		return NULL;
	}
...
static gboolean
gvm_statfs_check_space (const char *udi, statfs_mount_info *info, gpointer user_data)
{
	char *mount_point = NULL;
	...
	if (statvfs (mount_point, &buf) != -1) {
		...
		*/
		if (!info->notified && (free_space < config.percent_threshold && buf.f_bavail < twogb_blocks)) {
			...
			msg = g_strdup_printf (_("%d%% of the disk space on %s is in use"), free_pct, disk);
			g_free (disk);
			
			n = notify_notification_new ("Low Disk Space", msg, "gnome-dev-harddisk", NULL);
...
static gboolean
gvm_statfs_timeout (gpointer data)
{
